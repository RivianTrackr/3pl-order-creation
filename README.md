...
3. **Finds the client.** The PO's job names a Syncore contact; that contact's **client group** identifies the company, so store billing, employees and dropship contacts all map to one client on the **Clients** page. A client that isn't set up yet holds its POs (without using up retries) until someone finishes it.
...
| Ship Via | The warehouse's carriers and service codes loaded from 3PL Central (refreshed automatically every 6 hours), a tester that shows how any Ship Via text will be matched, and optional overrides for wording that doesn't match |
